...
use tessera_client::{
    AccountAddress, StandardAccount, SubpoolId,
...
pub async fn register_handler(
    ...
    let pi_bytes = hex::decode(&body.private_identifier)
        .map_err(|_| AppError::InvalidInput("private_identifier must be a valid hex string".into()))?;
    if pi_bytes.len() != 16 {
    ...
    let user_exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM users WHERE private_acc_address = $1)",
    )
