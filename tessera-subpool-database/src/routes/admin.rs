...
struct OutputNoteJoinRow {
	...
	// output_note_checks (LEFT JOIN — nullable)
	check_id: Option<i64>,
	check_identifier: Option<String>,
	check_status: Option<String>,
	check_response: Option<String>,
	check_updated_at: Option<DateTime<Utc>>,
	// users (LEFT JOIN on sender_address — nullable)
	name: Option<String>,
	physical_address: Option<String>,
	dob: Option<NaiveDate>,
}
...
#[derive(Debug, Serialize)]
pub struct OutputNoteCheckInfo {
	pub id: Option<i64>,
...
pub struct OutputNoteAdminRow {
	...
	pub created_at: DateTime<Utc>,
	pub updated_at: DateTime<Utc>,
	pub output_note_check: OutputNoteCheckInfo,
	pub sender: SenderInfo,
}

const OUTPUT_NOTE_JOIN_QUERY: &str = r#"
    SELECT
        n.id,
        n.identifier,
        n.asset_id,
        ...
        u.dob
    FROM output_notes n
    LEFT JOIN output_note_checks c ON c.output_note_id = n.id
    LEFT JOIN users u ON u.private_acc_address = n.sender_address
"#;

fn map_output_note_join_row(r: OutputNoteJoinRow) -> OutputNoteAdminRow {
	OutputNoteAdminRow {
		asset_id: hex::encode(&r.asset_id),
...
pub async fn list_underreview_output_notes_handler(
	State(state): State<AppState>,
...
pub async fn list_all_output_notes_handler(
	State(state): State<AppState>,
) -> Result<(StatusCode, Json<Vec<OutputNoteAdminRow>>), AppError> {
	let query = format!("{OUTPUT_NOTE_JOIN_QUERY} ORDER BY n.created_at DESC");
	let rows: Vec<OutputNoteJoinRow> = sqlx::query_as(&query).fetch_all(&state.pool).await?;
	Ok((StatusCode::OK, Json(rows.into_iter().map(map_output_note_join_row).collect())))
}

pub async fn review_output_note_handler(
	State(state): State<AppState>,
	Path(id): Path<i64>,
	Json(req): Json<ReviewDepositRequest>,
) -> Result<(StatusCode, Json<ReviewDepositResponse>), AppError> {
	...
	sqlx::query(
		"UPDATE output_notes \
         SET status = $1::output_note_status, updated_at = NOW() \
         WHERE id = $2",
	)
	.bind(new_status_pg)
	.bind(id)
