...
import {DoxaBatchTransactionVerifier} from "../src/DoxaBatchTransactionVerifier.sol";
...
contract DoxaRollupV2IntegrationTest is Test {
    ...
    function _loadBatch() internal returns (bytes memory preimage) {
        ...
        preimage = abi.encodePacked(
            ...
            _glH(doxa_contract.mainPoolConfigRoot())
        );
    ...
    function test_groth16_public_inputs_match_piCommitment() public {
        ...
        uint256[8] memory unpacked = doxa_contract.keccakToPublicInputs(piCommitment);
        for (uint256 i = 0; i < 8; i++) {
    ...
    function test_groth16_prove_transaction_batch() public {
        ...
        vm.prank(OP);
        doxa_contract.submitTransactionBatch(preimage);

        // Prove phase — uses the real gnark verifier.
        DoxaContract.Proof memory p = DoxaContract.Proof({
            proof:         proof,
            commitments:   commitments,
            commitmentPok: commitmentPok
        });
        doxa_contract.proveTransactionBatch(preimage, p);

        assertEq(doxa_contract.imtLeafCount(), 1, "leaf appended");
    }
}
...
/// @notice Requires:  cargo run -p doxa-e2e --bin deposit_artifacts --release
///         Fixture:   doxa-solidity/test/fixtures/groth16_deposit_proof.json
///         Verifier:  doxa-solidity/src/VerifierDepositSuperAggregatorV2.sol
contract DoxaDepositIntegrationTest is Test {
