...
use super::{CircuitDataBN128, ConfigBN128, ProofBN128, serializer::DoxaGeneratorSerializer};
...
impl BN128Wrapper {
	...
	pub fn store_full_circuit_data(&self, path: &Path) -> Result<()> {
		...
		let native_bytes = self
			.circuit_data
			.to_bytes(&gate_ser, &DoxaGeneratorSerializer)
			.map_err(|_| {
				anyhow!(
					"serialize native circuit failed (plonky2 IoError — no inner detail available). \
				 DoxaGeneratorSerializer covers all 24 standard generators plus the 10 custom \
				 Keccak-256 / SHA-256 / u32 generators. If a new custom generator was added to the circuit, \
				 register it in doxa-server/src/groth/serializer.rs."
				)
			})?;
	...
	pub fn from_artifacts(path: &Path) -> Result<Self> {
		...
		let circuit_data =
			CircuitDataNative::from_bytes(&native_bytes, &gate_ser, &DoxaGeneratorSerializer)
				.map_err(|_| {
					anyhow!(
			 ...
			 (3) a generator type present at serialization time is missing from DoxaGeneratorSerializer. \
			 Delete the artifacts directory and re-run commitment_tree_artifacts.",
