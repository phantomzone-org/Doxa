...
impl StandardAccount {
	...
	/// Compute the spend-once nullifier for this account.
	///
	/// `nullifier = H(commitment || nk)`
	///
	/// The `pos` argument is unused in the current implementation but
	/// is kept for forward-compatibility with position-dependent nullifiers.
	/// Publishing the nullifier proves the account has been consumed without
	/// revealing which account it was.
