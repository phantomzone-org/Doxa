...
interface KycRecord {
  name: string; street: string; dob: string;
  tesseraAddress: string; registeredAt: string;
}
...
async function registerAndGetSeed(): Promise<Uint8Array> {
  const cred = (await navigator.credentials.create({
    publicKey: {
      ...
      user: { id: crypto.getRandomValues(new Uint8Array(16)), name: "demo", displayName: "Demo User" },
...
async function evalPrf(): Promise<Uint8Array> {
  ...
  if (!credentialId) credentialId = new Uint8Array(assertion.rawId);
  const ext = assertion.getClientExtensionResults() as any;
  const prfOutput: ArrayBuffer | undefined = ext?.prf?.results?.first;
  if (!prfOutput) throw new Error("WebAuthn PRF extension unavailable");
  const seed = new Uint8Array(prfOutput);
  if (seed.byteLength !== 32) throw new Error(`PRF output must be 32 bytes, got ${seed.byteLength}`);
  return seed;
}
...
async function deriveEthAddress(seed: Uint8Array): Promise<string> {
  ...
  input.set(seed); input.set(domain, seed.length);
...
function pStep(container: HTMLElement, text: string, cls: "active" | "done"): HTMLElement {
...
const etherscanAnchor = document.getElementById("etherscan-anchor")  as HTMLAnchorElement;

let publicBalance = 0;

function renderPublicBalance() {
  usdcBalanceEl.textContent = publicBalance.toLocaleString("en-US", { minimumFractionDigits: 2 }) + " USDX";
}

async function showWallet(seed: Uint8Array) {
  ethAddressFull = await deriveEthAddress(seed);
  ethAddressEl.textContent = ethAddressFull.slice(0, 10) + "…" + ethAddressFull.slice(-8);
...
createWalletBtn.addEventListener("click", async () => {
  createWalletBtn.disabled = true; signInBtn.disabled = true;
  ...
  try {
    ...
    createWalletBtn.disabled = false; signInBtn.disabled = false;
    createWalletBtn.textContent = "🔑 Create Wallet";
  }
});

signInBtn.addEventListener("click", async () => {
  signInBtn.disabled = true; createWalletBtn.disabled = true;
  walletError.textContent = "";
  ...
  try {
    ...
    walletError.textContent = `Error: ${err}`;
    signInBtn.disabled = false; createWalletBtn.disabled = false;
...
depositBtn.addEventListener("click", async () => {
  ...
  for (const step of steps) {
    if (prev) { prev.className = "p-step done"; prev.textContent = "✓ " + prev.textContent!.replace(/^⏳ /, ""); }
    const el = pStep(depositSteps, (step.pct < 100 ? "⏳ " : "") + step.label, step.pct < 100 ? "active" : "done");
...
function renderPrivateBalance() {
  privBalanceEl.textContent = privateBalance.toLocaleString("en-US", { minimumFractionDigits: 2 }) + " USDX";
...
function onSignedIn() {
  ...
  if (kyc) {
    ...
    dispName.textContent = kyc.name; dispStreet.textContent = kyc.street; dispDob.textContent = kyc.dob;
...
for (const input of [nameInput, streetInput, dobInput]) {
  input.addEventListener("input", () => { registerBtn.disabled = !formFilled(); });
}
function formFilled() {
  return nameInput.value.trim() !== "" && streetInput.value.trim() !== "" && dobInput.value !== "";
}
...
registerBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const s1 = appendProgressLine("⏳ Waiting for approval from subpool owner…", "active");
    await delay(1800);
    s1.className = "progress-line done"; s1.textContent = "✓ Approval received";

    const s2 = appendProgressLine("⚙️ Generating proof…", "active");
    await delay(2200);
    s2.className = "progress-line done"; s2.textContent = "✓ Proof generated";
    ...
    tesseraAddrBox.classList.add("visible");
    saveKyc({ name: nameInput.value.trim(), street: streetInput.value.trim(), dob: dobInput.value, tesseraAddress, registeredAt: new Date().toISOString() });
  } catch (err) {
...
const p2pAmountInput = document.getElementById("p2p-amount")   as HTMLInputElement;
...
p2pBtn.addEventListener("click", async () => {
  ...
  if (!amount || amount <= 0) { p2pError.textContent = "Enter a valid amount."; return; }
  if (!ethAddressFull) { p2pError.textContent = "Connect a wallet first."; return; }
  ...
  try {
    ...
    combined.set(seed); combined.set(txHash, seed.length);
    ...
    const steps: Array<{ label: string | (() => string | HTMLElement); pct: number; ms: number }> = [
    ...
    for (const step of steps) {
      ...
      if (step.ms > 0) {
        await delay(step.ms);
        if (step.pct < 100 && typeof step.label === "string" && step.label.startsWith("⏳")) {
          el.className = "p-step done";
          el.textContent = step.label.replace("⏳ ", "✓ ").replace("…", "");
        } else if (step.pct < 100 && typeof step.label === "string" && step.label.startsWith("⚙️")) {
          el.className = "p-step done";
...
const DEMO_ADDRESSES = [
  { label: "Alice",
    addr: "0000000000000001" + "0000000000000001" + "0000000000000002" + "0000000000000003" + "0000000000000004" },
  { label: "Bob",
    addr: "0000000000000002" + "0000000000000005" + "0000000000000006" + "0000000000000007" + "0000000000000008" },
  { label: "Charlie",
    addr: "0000000000000003" + "0000000000000009" + "000000000000000a" + "000000000000000b" + "000000000000000c" },
];

function validateTesseraAddr(hex: string): boolean {
  try { AccountAddress.fromHex(hex); return true; }
  catch { return false; }
}
...
for (const { label, addr } of DEMO_ADDRESSES) {
  ...
  lbl.className = "addr-label"; lbl.textContent = label;
...
xferAddrIn.addEventListener("input", () => {
  ...
  if (v === "") {
    addrTick.classList.add("hidden"); addrHint.textContent = "";
  } else if (validateTesseraAddr(v)) {
    addrTick.classList.remove("hidden"); addrHint.textContent = "";
  } else {
    addrTick.classList.add("hidden");
    addrHint.textContent = "Invalid address (must be 80 hex chars with valid field elements)";
  }
  updateXferBtn();
});
...
xferBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const steps = [
      { label: "⏳ Waiting for approval from subpool owner…", pct: 33,  ms: 1800 },
      { label: "⚙️ Generating proof…",                       pct: 66,  ms: 2200 },
      { label: "✓ Send settled",                             pct: 100, ms: 0    },
    ];

    for (const step of steps) {
      const el = pStep(xferSteps, step.label, step.pct < 100 ? "active" : "done");
