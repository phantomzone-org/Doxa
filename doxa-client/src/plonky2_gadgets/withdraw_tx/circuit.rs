...
///
/// A withdrawal moves up to `NOTE_BATCH` asset balances from a Doxa account
...
// 	pub fn prove_dummy(&self) -> doxa_utils::ProofNative {
...
pub fn build_withdraw_tx_circuit() -> WithdrawTxCircuit {
	...
	let mut builder = CircuitBuilder::<doxa_utils::F, { doxa_utils::D }>::new(config);
	let targets =
		withdraw_tx_circuit::<HashOutput, doxa_utils::F, { doxa_utils::D }>(&mut builder);
	let circuit_data = builder.build::<doxa_utils::ConfigNative>();
	WithdrawTxCircuit {
		circuit_data,
		targets,
	}
