...
pub fn build_deposit_tx_circuit() -> DepositTxCircuit {
	...
	let mut builder = CircuitBuilder::<F, { doxa_utils::D }>::new(config);
	let targets = deposit_tx_circuit::<HashOutput, F, { doxa_utils::D }>(&mut builder);
...
/// Doxa account, with all authorization checks satisfied.
///
