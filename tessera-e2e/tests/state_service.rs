...
//! # What is tested
//!
//! - Genesis sync: StateService started *after* batches are already on-chain replicates them
//!   correctly into its local flat tree.
//! - Incremental sync: StateService polls and picks up batches proven *while* it is running.
//! - Leaf-index lookups: note and account commitments map to the expected zero-based positions.
//! - Merkle proofs: siblings returned by the service satisfy [`MerkleProof::verify`].
...
mod helpers {
	...
	pub fn commitment_bytes(v: U256) -> [u8; 32] {
		let h =
			contract::u256_le_to_hash(v).expect("commitment_bytes: not a valid Goldilocks hash");
	...
	pub async fn submit_and_prove_tx_batch<P: Provider + Clone>(
		...
		let current_root = rollup.currentRoot().call().await.expect("currentRoot");
	...
	fn decode_tx_submitted_pi(receipt: &alloy::rpc::types::TransactionReceipt) -> B256 {
		receipt
			.inner
			.logs()
			.iter()
	...
	fn decode_deposit_submitted_pi(receipt: &alloy::rpc::types::TransactionReceipt) -> B256 {
		receipt
			.inner
			.logs()
			.iter()
...
async fn genesis_sync_empty_chain() {
	...
	let result = handle
		.get_leaf_index([1u8; 32])
		.await
		.expect("get_leaf_index failed");
...
async fn genesis_sync_single_tx_batch() {
	...
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(nc[0]))
			.await
			.expect("lookup nc[0]"),
		Some(0),
	...
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(nc[1]))
			.await
			.expect("lookup nc[1]"),
		Some(1),
	...
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(ac[0]))
			.await
			.expect("lookup ac[0]"),
		Some(2),
	...
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(nn[0]))
			.await
			.expect("lookup nn[0]"),
		None,
	...
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(an[0]))
			.await
			.expect("lookup an[0]"),
		None,
...
async fn genesis_sync_single_deposit_batch() {
	...
	helpers::register_and_prove_deposit_batch(&provider, ctx.rollup_addr, ctx.token_addr, &dnc, 0)
		.await;
...
async fn genesis_sync_multiple_mixed_batches() {
	...
	helpers::register_and_prove_deposit_batch(&provider, ctx.rollup_addr, ctx.token_addr, &dnc, 1)
		.await;
	...
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(helpers::gl_commitment(1)))
			.await
			.unwrap(),
		Some(0)
	);
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(helpers::gl_commitment(2)))
			.await
			.unwrap(),
		Some(1)
	);
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(helpers::gl_commitment(3)))
			.await
			.unwrap(),
		Some(2)
	);
	...
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(helpers::gl_commitment(4)))
			.await
			.unwrap(),
		Some(4)
	);
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(helpers::gl_commitment(5)))
			.await
			.unwrap(),
		Some(5)
	);
	assert_eq!(
		handle
			.get_leaf_index(helpers::commitment_bytes(helpers::gl_commitment(6)))
			.await
			.unwrap(),
		Some(6)
	);
...
async fn unknown_commitment_returns_none() {
	...
	assert_eq!(
		handle
			.get_leaf_index([0u8; 32])
			.await
			.expect("lookup zero bytes"),
		None,
