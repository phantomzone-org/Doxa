...
//! `DoxaBatchTransactionVerifier` contract.
//!
//! # Requirements
//!
...
//!    doxa-solidity && forge build ```
//!
//! 2. **Generate PrivTxAggregator artifacts**: ```text cargo run -p doxa-server --bin priv_tx_setup
//!    --release ``` Artifacts are written to `doxa-server/artifacts/priv-tx/`.
//!
//! # Running
//!
//! ```text
//! cargo test -p doxa-e2e --release \
...
use doxa_client::{
	...
	schnorr::PrivateKey,
	DoxaGateSerializer, HashOutput, PIHelper, PrivTxProof, StandardAccount, SubpoolId,
...
// ---------------------------------------------------------------------------
// Alloy on-chain interface (DoxaContract — new 8-param constructor)
// ---------------------------------------------------------------------------
...
fn workspace_root() -> std::path::PathBuf {
	// CARGO_MANIFEST_DIR = <workspace>/doxa-e2e
...
fn load_foundry_bytecode(sol_file: &str, contract: &str) -> String {
	...
	assert!(
		...
		"Foundry artifact not found at {path:?}.\n\
		 Run:  cd doxa-solidity && forge build",
	);
...
async fn test_e2e_setup_operators_and_freshacc_batch() {
	...
	let rollup_bytecode = load_foundry_bytecode("DoxaContract.sol", "DoxaContract");
	...
	let verifier_bytecode = load_foundry_bytecode(
		"DoxaBatchTransactionVerifier.sol",
		"DoxaBatchTransactionVerifier",
	);

	let ws = workspace_root();
	let agg_path = ws.join("doxa-server/artifacts/priv-tx");
	let plonky2_path = agg_path.join("plonky2-proof");
	let groth_path = agg_path.join("groth-artifacts");

	const GEN_CMD: &str = "  cargo run -p doxa-server --bin priv_tx_artifacts --release";
	...
	// DoxaContract(txVerifier, bridgeTxVerifier, poseidon, operator,
	...
	let rollup = IDoxa::IDoxaInstance::new(rollup_addr, &op_provider);
	...
	for o in &owners {
		...
		let spowner_provider = ProviderBuilder::new()
			...
			.connect_http(anvil.endpoint_url());
		IDoxa::IDoxaInstance::new(rollup_addr, &spowner_provider)
			.updateSubpoolRoot(
