    ...
    try {
        ...
        if (firstResult) {
            ...
            if (seed.byteLength !== 32)
                throw new Error(
                    `PRF output must be 32 bytes, got ${seed.byteLength}`,
                );
            return seed;
        } else {
...
async function loadPrivateAccount(seed: Uint8Array) {
    ...
    subpoolClient
        .getUser(privateAccAddress)
        .then((user) => {
            kycInfo = user;
            renderPrivateSection();
        })
        .catch((e) => console.log("getUser failed:", e));
}
...
function renderPrivateSection() {
    if (privateAccount) {
        ...
        const notesAmountHex =
            notesBalance?.balances[ASSET_ID.toString()]?.amount;
    ...
    try {
        ...
        await new Promise<void>((resolve, reject) => {
            const timer = setInterval(async () => {
                try {
                    const status =
                        await subpoolClient.getDepositStatus(depositId);
...
xferBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        await new Promise<void>((resolve, reject) => {
            const timer = setInterval(async () => {
                try {
                    const status = await subpoolClient.getSpendTxStatus(
                        resp.id,
                    );
                    if (!status || status.status === "Pending") return;
                    clearInterval(timer);
