...
impl StateSyncService {
    ...
    pub async fn sync_from_genesis<P: Provider + Clone>(
        ...
        info!("Starting genesis sync from block {} to {}", genesis_block, latest_block);
        ...
        process_new_submissions(&provider, address, genesis_block, latest_block, chunk_blocks, &service).await?;

        // Collect and replay proven batches
        let mut proven_batches = collect_proven_batches(&provider, address, genesis_block, latest_block, chunk_blocks).await?;
        proven_batches.sort_by_key(|b| b.leaf_index);

        info!(total_proven_batches = proven_batches.len(), "Replaying proven batches");
        ...
        for entry in &proven_batches {
            ...
            let preimage_known = service.with_state(|s| {
                s.pending_tx_batches.contains_key(&entry.pi_commitment.0)
                    || s.pending_bridge_tx_batches.contains_key(&entry.pi_commitment.0)
            });
            ...
            if !preimage_known {
                ...
                let tx_hash = find_submission_tx_hash(
                    ...
                    entry.pi_commitment, genesis_block, latest_block, chunk_blocks,
            ...
            // Confirm the batch
            let batch_root = service.with_state(|s| {
                let batches = match entry.kind {
                    BatchKind::Transaction => &s.pending_tx_batches,
                    BatchKind::BridgeTx => &s.pending_bridge_tx_batches,
                };
                batches.get(&entry.pi_commitment.0).map(|p| extract_batch_root_from_preimage(p, entry.kind))
            }).transpose()?;

            if let Some(batch_root) = batch_root {
                service.with_state_mut(|s| s.confirm_batch(entry.pi_commitment.0, batch_root, entry.new_tree_root))?;
            }
        }

        // Sync config tree
        sync_config_tree(&provider, address, genesis_block, latest_block, chunk_blocks, &service, genesis_block).await?;

        // Sync deposits
        sync_deposits(&provider, address, genesis_block, latest_block, chunk_blocks, &service).await?;
    ...
    pub async fn poll_sync<P: Provider + Clone>(
        ...
        process_new_submissions(&provider, address, from_block + 1, to_block, chunk_blocks, self).await?;

        // Step 2: collect and apply proven batches
        let mut proven = collect_proven_batches(&provider, address, from_block + 1, to_block, chunk_blocks).await?;
        ...
        for entry in &proven {
            ...
            // Confirm the batch
            let batch_root = self.with_state(|s| {
                let batches = match entry.kind {
                    BatchKind::Transaction => &s.pending_tx_batches,
                    BatchKind::BridgeTx => &s.pending_bridge_tx_batches,
                };
            ...
            if let Some(batch_root) = batch_root {
                self.with_state_mut(|s| s.confirm_batch(entry.pi_commitment.0, batch_root, entry.new_tree_root))?;
            }
        }

        // Step 3: config tree and deposits
        sync_config_tree(&provider, address, from_block + 1, to_block, chunk_blocks, self, 0).await?;
        sync_deposits(&provider, address, from_block + 1, to_block, chunk_blocks, self).await?;
...
async fn process_new_submissions<P: Provider + Clone>(
    ...
    let tx_submit_map = build_tx_submit_map(provider, address, from_block, to_block, chunk_blocks).await?;
    let bridge_submit_map = build_bridge_submit_map(provider, address, from_block, to_block, chunk_blocks).await?;

    for (pi_commitment, tx_hash) in &tx_submit_map {
        let already_known = service.with_state(|s| s.pending_tx_batches.contains_key(&pi_commitment.0) || s.confirmed_tx_batches.contains(&pi_commitment.0));
    ...
    for (pi_commitment, tx_hash) in &bridge_submit_map {
        let already_known = service.with_state(|s| s.pending_bridge_tx_batches.contains_key(&pi_commitment.0) || s.confirmed_bridge_tx_batches.contains(&pi_commitment.0));
        if already_known { continue; }
        let input = fetch_transaction_input(provider, *tx_hash).await?;
        let preimage = decode_bridge_batch_calldata(&input)?;
...
async fn sync_config_tree<P: Provider + Clone>(
        ...
        if let Ok(decoded) = log.log_decode::<ITesseraRollupV2::SubpoolOwnerAssigned>() {
            ...
            service.with_state_mut(|state| {
                if event.subpool_id == state.next_expected_subpool_id {
                    ...
                    state.pending_subpool_assignments.insert(event.subpool_id, event);
                }
            });
        }
    }
    ...
    if from_block == genesis_block {
        let registered_subpool_ids: Vec<u64> = service.with_state(|s| (1..s.next_expected_subpool_id).collect());
        for subpool_id in registered_subpool_ids {
            let instance = ITesseraRollupV2::ITesseraRollupV2Instance::new(address, provider);
            let root_u256 = instance.subpoolRoots(subpool_id).call().await
    ...
    for log in &updated_logs {
        if let Ok(decoded) = log.log_decode::<ITesseraRollupV2::SubpoolRootUpdated>() {
            ...
            service.with_state_mut(|state| {
                state.update_subpool_root(subpool_id, new_root)
            })?;
        }
    }
    ...
    service.with_state_mut(|state| {
        ...
        for s in 0..PRIV_TX_BATCH_SIZE {
            ...
            for j in 0..NOTE_BATCH {
                ...
                state.add_pending_commitment(bytes_to_hash(&note_comm), pi_commitment, s * 8 + j + 1);
            }
        }
        ...
        // Store pending batch
        state.pending_tx_batches.insert(pi_commitment, alloy::primitives::Bytes::from(preimage.to_vec()));
    });
...
fn apply_bridge_preimage(
    ...
    service.with_state_mut(|state| {
        ...
        // Store pending batch
        state.pending_bridge_tx_batches.insert(pi_commitment, alloy::primitives::Bytes::from(preimage.to_vec()));
    });
...
fn decode_tx_proven_log(log: &Log) -> anyhow::Result<ProvenBatchEntry> {
    ...
    Ok(ProvenBatchEntry {
        ...
        leaf_index: inner.leafIndex.try_into().context("leafIndex overflows u64")?,
...
fn decode_tx_batch_calldata(input: &alloy::primitives::Bytes) -> anyhow::Result<alloy::primitives::Bytes> {
    use alloy::sol_types::SolCall;
    let call = ITesseraRollupV2::submitTransactionBatchCall::abi_decode(input)?;
    Ok(call.batchPreimage)
}

fn decode_bridge_batch_calldata(input: &alloy::primitives::Bytes) -> anyhow::Result<alloy::primitives::Bytes> {
    use alloy::sol_types::SolCall;
...
async fn fetch_config_tree_depth<P: Provider + Clone>(
    ...
    let instance = ITesseraRollupV2::ITesseraRollupV2Instance::new(address, provider);
    let depth = instance.configTreeDepth().call().await.context("configTreeDepth()")?;
    ...
    match fetch_on_chain_leaf_count(provider, address).await {
        Ok(on_chain) => {
            ...
            if local_count == on_chain {
                info!(leaf_count = local_count, "local tree matches on-chain leaf count");
            } else {
                warn!(
                    local = local_count,
