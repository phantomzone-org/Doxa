...
A standalone HTTP service that tracks on-chain state from `TesseraContract` and provides real-time access to:
...
- A deployed `TesseraContract` instance
...
TESSERA_CONTRACT_ADDRESS="0x..."

# Optional (defaults shown)
TESSERA_STATE_SYNC_POLL_INTERVAL="12"  # seconds
TESSERA_STATE_SYNC_BIND_ADDR="0.0.0.0:3001"
```

2. **Run the service**:

```bash
cd tessera-state-sync
cargo run --release
```
...
```bash
# Info level (default)
RUST_LOG=tessera_state_sync=info cargo run --release

# Debug level (more detail)
RUST_LOG=tessera_state_sync=debug cargo run --release

# Trace level (very verbose)
RUST_LOG=tessera_state_sync=trace cargo run --release
```
...
```ini
[Unit]
Description=Tessera State Sync Service
...
ExecStart=/opt/tessera/tessera-state-sync
Restart=on-failure
RestartSec=10s
StandardOutput=journal
StandardError=journal

Environment="TESSERA_RPC_URL=http://localhost:8545"
Environment="TESSERA_CONTRACT_ADDRESS=0x..."
Environment="TESSERA_STATE_SYNC_POLL_INTERVAL=12"
Environment="TESSERA_STATE_SYNC_BIND_ADDR=0.0.0.0:3001"
Environment="RUST_LOG=tessera_state_sync=info"
...
- Invalid `TESSERA_RPC_URL` (network timeout, malformed URL)
- Invalid `TESSERA_CONTRACT_ADDRESS` (wrong address, not deployed)
- RPC endpoint rate-limited or down

**Fix**:
1. Verify RPC URL is reachable: `curl $TESSERA_RPC_URL`
...
**Fix**:
1. Change `TESSERA_STATE_SYNC_BIND_ADDR` to a different port
...
**Fix**:
1. Check logs for sync status: `RUST_LOG=tessera_state_sync=debug`
...
1. Check on-chain root via `TesseraContract.root()` call
