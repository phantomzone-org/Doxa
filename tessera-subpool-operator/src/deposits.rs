...
use tessera_subpool_database::{
    convert::{
        account_from_row, build_ast_json, bytes_to_f, bytes_to_u256,
        f_to_bytes, hash_to_hex, parse_eth_address, u256_to_bytes,
    },
...
async fn broadcast_deposit_tx<P: Provider + Clone>(
    ...
    anyhow::ensure!(receipt.status(), "deposit tx reverted on-chain (tx={tx_hash})");
...
fn parse_deposit_fields(row: &DepositTxRow) -> Result<ParsedDeposit> {
    let note_id_arr: [u8; 16] = row.deposit_note_identifier.as_slice().try_into()
    ...
    let amount_arr: [u8; 32] = row.deposit_amount.as_slice().try_into()
        .context("deposit_amount must be 32 bytes")?;
    let deposit_amount = bytes_to_u256(&amount_arr);

    let asset_id_arr: [u8; 8] = row.asset_id.as_slice().try_into()
    ...
    Ok(ParsedDeposit { note_identifier, deposit_amount, asset_id, asset_id_u64, eth_address })
}

/// Build the deposit note and compute its Poseidon commitment hash.
fn build_deposit_note(
...
fn apply_deposit(accin: &StandardAccount, deposit: &ParsedDeposit) -> StandardAccount {
    ...
    let old_balance = accout.ast.amount_for(deposit.asset_id).map(|(_, b)| b).unwrap_or(U256::zero());
    let new_balance = old_balance + deposit.deposit_amount;
    accout.ast.insert_or_update_asset(deposit.asset_id, new_balance);
    accout.balance = accin.balance + deposit.deposit_amount;
    accout
}
...
pub async fn process_pending_deposits<P: Provider + Clone>(
    ...
    for row in rows {
        if let Err(e) = process_one_deposit(pool, approval_sk, sequencer_url, http, rpc_provider, &row, subpool_id).await {
            error!(
                id = row.id,
...
async fn process_one_deposit<P: Provider + Clone>(
    ...
    let deposit_note_comm = tessera_client::DepositNoteCommitment(
        tessera_client::HashOutput(nc_hash),
    );
    ...
    let new_asset_balance = accout.ast.amount_for(deposit.asset_id)
        .map(|(_, b)| b).unwrap_or(U256::zero());
    let ast_json = build_ast_json(&acc_row.ast, deposit.asset_id_u64, new_asset_balance);

    update_account_after_deposit(
        pool,
    ...
    let nc_bytes: Vec<u8> = nc_hash.iter().flat_map(|f| f.to_canonical_u64().to_be_bytes()).collect();
