...
pub async fn process_pending_deposits<P: Provider + Clone>(
    ...
    for row in rows {
        if let Err(e) = process_one_deposit(pool, approval_sk, sequencer_url, http, rpc_provider, &row).await {
            error!(
                id = row.id,
...
async fn process_one_deposit<P: Provider + Clone>(
    ...
    info!(
        ...
        "deposit approved and account updated"
    );
