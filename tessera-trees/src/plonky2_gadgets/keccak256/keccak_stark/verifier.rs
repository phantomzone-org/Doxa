...
use starky::{
	config::StarkConfig,
	cross_table_lookup::{
		CrossTableLookup, CtlCheckVars, CtlCheckVarsTarget, verify_cross_table_lookups,
...
where
{
	let mut ctl_challenger = Challenger::<F, C::Hasher>::new();
	ctl_challenger.observe_elements(public_inputs);
	let preliminary_challenges = proof.proof.get_challenges::<S>(
		stark,
		public_inputs,
		&mut ctl_challenger,
		None,
	...
	let ctl_challenges = preliminary_challenges.lookup_challenge_set.unwrap();
	...
	let stark_challenges = proof.proof.get_challenges::<S>(
		stark,
		public_inputs,
		&mut challenger,
		Some(&ctl_challenges),
	...
	C::Hasher: AlgebraicHasher<F>,
{
	...
	let mut ctl_challenger = RecursiveChallenger::<F, C::Hasher, D>::new(builder);
	for x in &stark_proof_with_pi_target.public_inputs {
		ctl_challenger.observe_element(*x);
	}
	let preliminary_challenges = stark_proof_target.get_challenges::<F, C, S>(
		builder,
		stark,
...
mod tests {
	...
	use crate::plonky2_gadgets::keccak256::keccak_stark::{
