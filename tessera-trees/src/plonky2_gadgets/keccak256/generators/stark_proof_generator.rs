...
use crate::plonky2_gadgets::keccak256::{
	...
	utils::{BLOCK_SIZE, solidity_keccak256_with_perm_io},
};
	...
	C::Hasher: AlgebraicHasher<F>,
{
	...
	fn run_once(
		...
		let stark_proof =
			crate::plonky2_gadgets::keccak256::keccak_stark::prover::prove::<F, C, _, D>(
				&stark,
				&config,
				&trace,
				&cross_table_lookups,
			...
			.unwrap();
		crate::plonky2_gadgets::keccak256::keccak_stark::verifier::verify(
			&stark,
			&config,
			&cross_table_lookups,
			&stark_proof,
		...
		.unwrap();
		let pis_degree_bits = stark_proof.proof.recover_degree_bits(&config);
