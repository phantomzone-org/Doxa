...
impl NullifierTree<HashOutput> {
	...
	///
	/// ```text
	///   value[i] = HashOutput::from_32bytes_digest(Keccak256(i || H(node[i-1])))
	/// ```
	///
	...
	///   `HashOutput::from_32bytes_digest(Keccak256(leaf_index_be8 || node_hash_be32))`
...
mod tests {
	...
	/// Very large labels should still behave correctly (ordering comparisons on HashOutput bytes).
	#[test]
