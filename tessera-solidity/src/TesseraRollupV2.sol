...
contract TesseraRollupV2 {
    ...
    struct TransactionBatch {
        ...
        uint256   accountCommitment;   // account commitment for this batch
        uint256   accountNullifier;    // account nullifier consumed
        uint256   batchPoseidonRoot;   // Poseidon root of noteCommitments subtree; inserted as leaf on prove
        bool      confirmed;
    }
        ...
        whenNotPaused
    {
        ...
        uint256 len = batch.noteNullifiers.length;
        for (uint256 i = 0; i < len; i++) {
            if (nullifiers[batch.noteNullifiers[i]]) revert NullifierAlreadyUsed(batch.noteNullifiers[i]);
        }
        if (nullifiers[batch.accountNullifier]) revert NullifierAlreadyUsed(batch.accountNullifier);
        ...
        nullifiers[batch.accountNullifier] = true;
