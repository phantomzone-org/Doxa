...
use doxa_utils::hasher::{HashOutput, MerkleHash};
use tracing::{debug, info, warn};

use crate::{
	constants::*,
	contract::{self, IDoxaRollupV2},
	state::*,
};
...
impl StateSyncService {
	...
	pub async fn sync_from_genesis<P: Provider + Clone>(
		...
		let genesis_block: u64 = std::env::var("DOXA_GENESIS_BLOCK")
			.ok()
		...
		anyhow::ensure!(
			config_tree_depth == doxa_client::MAIN_POOL_CONFIG_DEPTH,
			"on-chain configTreeDepth={} != MAIN_POOL_CONFIG_DEPTH={}",
			config_tree_depth,
			doxa_client::MAIN_POOL_CONFIG_DEPTH
		);
				...
				let tx_hash = find_submission_tx_hash(
					...
					match entry.kind {
						BatchKind::Transaction => {
							IDoxaRollupV2::TransactionBatchSubmitted::SIGNATURE_HASH
						},
						BatchKind::BridgeTx => {
							IDoxaRollupV2::BridgeTxBatchSubmitted::SIGNATURE_HASH
						},
					},
					entry.pi_commitment,
					genesis_block,
...
async fn collect_proven_batches<P: Provider + Clone>(
	...
	let tx_proven_logs = fetch_logs(
		provider,
		address,
		IDoxaRollupV2::TransactionBatchProven::SIGNATURE_HASH,
		from_block,
		to_block,
		chunk_blocks,
	...
	let bridge_proven_logs = fetch_logs(
		provider,
		address,
		IDoxaRollupV2::BridgeTxBatchProven::SIGNATURE_HASH,
		from_block,
		to_block,
		chunk_blocks,
		"BridgeTxBatchProven",
...
async fn sync_config_tree<P: Provider + Clone>(
	...
	let assigned_logs = fetch_logs(
		provider,
		address,
		IDoxaRollupV2::SubpoolOwnerAssigned::SIGNATURE_HASH,
		from_block,
		to_block,
		chunk_blocks,
	...
	let updated_logs = fetch_logs(
		provider,
		address,
		IDoxaRollupV2::SubpoolRootUpdated::SIGNATURE_HASH,
		from_block,
		to_block,
		chunk_blocks,
		"SubpoolRootUpdated",
	...
	for log in &assigned_logs {
		if let Ok(decoded) = log.log_decode::<IDoxaRollupV2::SubpoolOwnerAssigned>() {
	...
	if from_block == genesis_block {
		...
		for subpool_id in registered_subpool_ids {
			let instance = IDoxaRollupV2::IDoxaRollupV2Instance::new(address, provider);
		...
		for log in &updated_logs {
			if let Ok(decoded) = log.log_decode::<IDoxaRollupV2::SubpoolRootUpdated>() {
	...
	service.with_state_mut(|s| {
		s.config_tree = doxa_client::pool_config::MainPoolConfigTree::new();
...
async fn sync_deposits<P: Provider + Clone>(
	...
	let deposit_available_logs = fetch_logs(
		provider,
		address,
		IDoxaRollupV2::DepositAvailable::SIGNATURE_HASH,
		from_block,
		to_block,
		chunk_blocks,
		"DepositAvailable",
	...
	let deposit_validated_logs = fetch_logs(
		provider,
		address,
		IDoxaRollupV2::DepositValidated::SIGNATURE_HASH,
		from_block,
		to_block,
		chunk_blocks,
		"DepositValidated",
	...
	let deposit_withdrawn_logs = fetch_logs(
		provider,
		address,
		IDoxaRollupV2::DepositWithdrawn::SIGNATURE_HASH,
		from_block,
		to_block,
		chunk_blocks,
		"DepositWithdrawn",
	...
	for log in &deposit_available_logs {
		if let Ok(decoded) = log.log_decode::<IDoxaRollupV2::DepositAvailable>() {
	...
	for log in &deposit_validated_logs {
		if let Ok(decoded) = log.log_decode::<IDoxaRollupV2::DepositValidated>() {
			let note_commitment = decoded.inner.noteCommitment.0;
	...
	for log in &deposit_withdrawn_logs {
		if let Ok(decoded) = log.log_decode::<IDoxaRollupV2::DepositWithdrawn>() {
			let note_commitment = decoded.inner.noteCommitment.0;
...
fn bytes_to_hash(b: &[u8; 32]) -> doxa_utils::hasher::HashOutput {
...
fn decode_tx_submitted_log(log: &Log) -> anyhow::Result<(B256, B256)> {
	let decoded = log
		.log_decode::<IDoxaRollupV2::TransactionBatchSubmitted>()
...
fn decode_bridge_submitted_log(log: &Log) -> anyhow::Result<(B256, B256)> {
	let decoded = log
		.log_decode::<IDoxaRollupV2::BridgeTxBatchSubmitted>()
...
fn decode_tx_proven_log(log: &Log) -> anyhow::Result<ProvenBatchEntry> {
	let decoded = log
		.log_decode::<IDoxaRollupV2::TransactionBatchProven>()
...
fn decode_bridge_proven_log(log: &Log) -> anyhow::Result<ProvenBatchEntry> {
	let decoded = log
		.log_decode::<IDoxaRollupV2::BridgeTxBatchProven>()
...
fn decode_tx_batch_calldata(
	...
	let call = IDoxaRollupV2::submitTransactionBatchCall::abi_decode(input)?;
...
fn decode_bridge_batch_calldata(
	...
	use alloy::sol_types::SolCall;
	let call = IDoxaRollupV2::submitBridgeTxBatchCall::abi_decode(input)?;
	Ok(call.batchPreimage)
}
