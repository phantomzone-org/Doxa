...
use doxa_state_sync::{api::*, StateSyncService};
...
async fn main() -> anyhow::Result<()> {
    ...
    let rpc_url = std::env::var("DOXA_RPC_URL")
        .context("DOXA_RPC_URL environment variable not set")?;
    let contract_address: Address = std::env::var("DOXA_CONTRACT_ADDRESS")
        .context("DOXA_CONTRACT_ADDRESS environment variable not set")?
        .parse()
        .context("invalid DOXA_CONTRACT_ADDRESS")?;

    let poll_interval_secs: u64 = std::env::var("DOXA_STATE_SYNC_POLL_INTERVAL")
        .unwrap_or_else(|_| "12".to_string())
        .parse()
        .context("invalid DOXA_STATE_SYNC_POLL_INTERVAL")?;

    let bind_addr = std::env::var("DOXA_STATE_SYNC_BIND_ADDR")
