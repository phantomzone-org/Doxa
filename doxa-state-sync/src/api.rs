...
fn hash_output_to_hex_string(hash: &doxa_utils::hasher::HashOutput) -> String {
...
pub async fn get_subpool_full_proof(
    ...
    let result = service.with_state(|state| {
        if let Some(&subpool_root) = state.subpool_roots.get(&params.subpool_id) {
            ...
            use doxa_client::pool_config::MainPoolConfigLeaf;
            use doxa_client::SubpoolId;
            use doxa_utils::F;
            use doxa_utils::hasher::MerkleHash;

            let subpool_id_field = SubpoolId(F::from_canonical_u64(params.subpool_id));
            let leaf_value = if subpool_root == doxa_utils::hasher::HashOutput::ZERO {
                doxa_utils::hasher::HashOutput::ZERO
            } else {
                let leaf = MainPoolConfigLeaf::<doxa_utils::hasher::HashOutput>::new(subpool_root, subpool_id_field);
                leaf.commit()
            };
