...
impl StateMirrorExpectation {
    ...
    fn assert(&self, service: &doxa_state_sync::StateSyncService) {
...
async fn test_bridge_batch_scenarios() {
    ...
    // Register asset 1
    IDoxaRollupV2::new(env.rollup, &provider)
    ...
    IDoxaRollupV2::new(env.rollup, &dep_provider)
...
async fn test_subpool_lifecycle() {
    ...
    let onchain_root = IDoxaRollupV2::new(env.rollup, &provider).mainPoolConfigRoot().call().await.unwrap();
...
async fn test_api_deposits() {
    ...
    // First deposit
    let receipt1 = IDoxaRollupV2::new(env.rollup, &dep_provider)
