...
use doxa_state_sync::{
    ...
    contract::{
        IDoxaRollupV2, IDoxaRollupV2::Proof, bytes32_to_hash, preimage_bytes32_to_raw,
    },
};
...
fn get_test_infra() -> &'static TestInfra {
    TEST_INFRA.get_or_init(|| {
        ...
        let solidity_dir = format!("{}/../doxa-solidity", manifest_dir);
        ...
        let out_dir = format!("{}/../doxa-solidity/out", manifest_dir);
...
// ---------------------------------------------------------------------------
// V2 DoxaContract bytecode loader
// ---------------------------------------------------------------------------

/// Load V2 DoxaContract deployment bytecode from the Foundry artifact.
/// Path: <manifest_dir>/../doxa-solidity/out/DoxaContract.sol/DoxaContract.json
pub fn v2_contract_bytecode() -> String {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let path = format!(
        "{}/../doxa-solidity/out/DoxaContract.sol/DoxaContract.json",
        manifest_dir
    );
    let text =
        std::fs::read_to_string(&path).unwrap_or_else(|e| panic!("Cannot read {}: {}", path, e));
...
async fn deploy_doxa_contract<P: Provider>(
    provider: &P,
    accept_all: Address,
    poseidon: Address,
    operator: Address,
    ...
    let _ = bytecodes; // bytecodes not used here; DoxaContract loaded from artifact
...
/// Deploy AcceptAllVerifier (×2), Poseidon, V2 DoxaContract, ToyUSDT.
...
/// doxa_client::MAIN_POOL_CONFIG_DEPTH = 20.
pub async fn setup_env() -> (TestEnv, impl Provider + Clone) {
    ...
    // Deploy DoxaContract (bytecode + ABI-encoded constructor args)
    let rollup =
        deploy_doxa_contract(&provider, accept_all, poseidon, operator, &infra.bytecodes).await;
...
pub fn genesis_act_root() -> HashOutput {
    doxa_trees::MerkleTree::<HashOutput>::new(23).root()
}
...
pub fn genesis_config_root() -> HashOutput {
    doxa_client::pool_config::MainPoolConfigTree::<HashOutput>::new().root()
}
...
pub async fn submit_tx_batch<P: Provider>(provider: &P, rollup: Address, preimage: &[u8]) {
    IDoxaRollupV2::new(rollup, provider)
...
pub fn patch_bridge_deposit_slot(
    ...
    use doxa_state_sync::constants::{D_NOTE_COMM_OFF, D_SECTION_OFF, D_SLOT_SIZE};

    // These offsets match DoxaContract.sol internal constants (not re-exported from
    // constants.rs)
...
pub fn patch_bridge_withdraw_slot(preimage: &[u8], slot_idx: usize) -> Vec<u8> {
    use doxa_state_sync::constants::{W_ACCIN_NULL_OFF, W_SLOT_SIZE};
    let mut out = preimage.to_vec();
...
pub fn patch_tx_slot_is_real(preimage: &[u8], slot_idx: usize) -> Vec<u8> {
    use doxa_state_sync::constants::{TX_ACCIN_NULL_OFF, TX_NOTE_IN_OFF};
    let mut out = preimage.to_vec();
