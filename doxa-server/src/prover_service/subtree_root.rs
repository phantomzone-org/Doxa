...
//! # Intended use
//!
//! In the DoxaContract design, N = 128 (depth 7). The `batch_poseidon_root`
...
impl SubtreeRootCircuit {
	...
	pub fn store_artifacts(&self, path: &Path) -> Result<()> {
		...
		let bytes = self
			.circuit_data
			.to_bytes(&DefaultGateSerializer, &DoxaGeneratorSerializer)
			.map_err(|_| {
				anyhow!(
...
mod tests {
	...
	use doxa_utils::hasher::{MerkleHash, NewRandom};
