...
use doxa_utils::{groth::DoxaGeneratorSerializer, ConfigNative, D, F};

// ---------------------------------------------------------------------------
// Artifact persistence — concrete types only
// ---------------------------------------------------------------------------
//
// `DoxaGeneratorSerializer` implements `WitnessGeneratorSerializer` only for
...
impl GenericAggregator<doxa_utils::F, doxa_utils::ConfigNative, 2> {
	...
	/// plonky2 gates, or a custom serializer (e.g. `DoxaGateSerializer`)
				...
				.map_err(|_| {
					anyhow!(
						"serialize level {i} circuit failed (plonky2 IoError). \
                         ...
                         doxa-trees/src/groth/serializer.rs."
					)
				})?;
			...
			.map_err(|_| {
				anyhow!(
					"deserialize level {i} circuit from '{}' failed (plonky2 IoError). \
                             ...
                             serialization time is missing from DoxaGeneratorSerializer. \
