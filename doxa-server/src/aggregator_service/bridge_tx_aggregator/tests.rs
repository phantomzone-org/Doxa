...
//
// Run with:
//   cargo test -p doxa-server --release -- --include-ignored bridge_tx_batch_to_groth16_e2e
#[test]
#[ignore]
fn bridge_tx_batch_to_groth16_e2e() {
	...
	use doxa_client::{
		build_deposit_tx_circuit, build_withdraw_tx_circuit, DoxaGateSerializer,
	};
	use doxa_utils::{
	...
	const GEN_CMD: &str = "  cargo run -p doxa-e2e --bin bridge_tx_artifacts --release";
