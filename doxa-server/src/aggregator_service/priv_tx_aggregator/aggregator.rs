...
use doxa_client::{FakeSpendTxBuilder, PIHelper, SUBTREE_BATCHSIZE};
use doxa_utils::{hasher::HashOutput, CircuitDataNative, ConfigNative, ProofNative, D, F};
...
/// agg.store_artifacts(Path::new("artifacts/priv-tx-agg"), &DoxaGateSerializer)?;
///
/// let agg = PrivTxAggregator::from_artifacts(
///     Path::new("artifacts/priv-tx-agg"), &DoxaGateSerializer,
/// )?;
...
impl PrivTxAggregator {
	...
	/// [`doxa_utils::groth::BN128Wrapper::new`].
	...
	pub fn prove_dummy(&self) -> Result<ProofNative> {
		use doxa_client::{build_priv_tx_circuit, PIHelper};
