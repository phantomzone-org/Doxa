...
//!
//! ```bash
//! DOXA_BRIDGE_TX_ARTIFACTS_PATH=artifacts/bridge-tx \
...
use anyhow::Result;
use doxa_client::{DoxaGateSerializer, build_deposit_tx_circuit, build_withdraw_tx_circuit};
use doxa_server::aggregator_service::BridgeTxAggregator;
...
fn main() -> Result<()> {
	...
	let root: PathBuf = std::env::var("DOXA_BRIDGE_TX_ARTIFACTS_PATH")
		.map(PathBuf::from)
	...
	let agg = if BridgeTxAggregator::has_full_artifacts(&root)? {
		...
		agg.store_artifacts(&root, &DoxaGateSerializer, &DoxaGateSerializer)?;
		agg
	};
