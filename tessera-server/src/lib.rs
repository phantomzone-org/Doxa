...
pub fn sample_batch_tree_proof(
	seed: [u8; 32],
) -> Result<(
	CircuitDataNative,
	ProofNative,
	...
	print!("Insert batch: ");
	let now = Instant::now();
	let mut batch = PendingDepositsBatch::new();
	for _ in 0..BATCH_SIZE {
		let mut note_commitment = [0u8; 32];
		let mut address = [0u8; 20];
		rng.fill_bytes(&mut note_commitment);
		rng.fill_bytes(&mut address);
		let deposit = PendingDeposit::new(note_commitment, address, rng.next_u64());
