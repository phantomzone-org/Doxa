...
///
/// Steps:
/// 1. Fetch `TransactionBatchSubmitted` / `DepositBatchSubmitted` events to
///    map each `piCommitment` to its submission transaction hash.
/// 2. Fetch `TransactionBatchProven` / `DepositBatchProven` events to obtain
///    the `leafIndex` and confirmed root for each proven batch.
/// 3. Merge both proven-event lists and sort by `leafIndex` (ascending) to
///    recover the canonical leaf-insertion order.
/// 4. For each batch, decode the original submission calldata, extract leaves
///    and nullifiers, and apply them to the snapshot.
...
pub async fn sync_from_genesis<P: Provider + Clone>(
    ...
    for batch_entry in &ordered_batches {
        replay_batch(provider, batch_entry, &tx_submit_map, &dep_submit_map, &mut state).await?;
    }
...
pub async fn sync_range<P: Provider + Clone>(
    ...
    for batch_entry in &ordered_batches {
        replay_batch(provider, batch_entry, &tx_submit_map, &dep_submit_map, state).await?;
    }
...
async fn replay_batch<P: Provider + Clone>(
    ...
    let tx_hash = submit_map.get(&entry.pi_commitment).copied().with_context(|| {
        format!(
    ...
    match fetch_on_chain_leaf_count(provider, address).await {
        Ok(on_chain) => {
            if state.leaf_count() as u64 == on_chain {
                info!(leaf_count = state.leaf_count(), "local tree matches on-chain leaf count");
            } else {
                warn!(
                    local = state.leaf_count(),
                    on_chain,
                    "local leaf count differs from on-chain leafCount()"
                );
            }
        }
