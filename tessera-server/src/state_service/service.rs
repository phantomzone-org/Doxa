...
impl StateService {
    ...
    pub fn new(config: StateServiceConfig) -> (Self, StateServiceHandle) {
        ...
        let handle = StateServiceHandle { tx };
        let service = Self {
    ...
    fn handle_request(&mut self, req: StateServiceRequest) {
        match req {
            StateServiceRequest::GetLeafIndex { commitment, reply } => {
                self.handle_get_leaf_index(commitment, reply);
            }
            StateServiceRequest::GetSiblings { commitment, reply } => {
    ...
    fn handle_get_siblings(
        &self,
        commitment: [u8; 32],
        reply: tokio::sync::oneshot::Sender<anyhow::Result<tessera_trees::MerkleProof<tessera_utils::hasher::HashOutput>>>,
    ) {
        let result = self
            .state
