...
//!
//! ```text
//! <TESSERA_PRIV_TX_ARTIFACTS_PATH>/
...
//!
//! ```bash
//! TESSERA_PRIV_TX_ARTIFACTS_PATH=artifacts/priv-tx \
...
use anyhow::Result;
use tessera_client::{TesseraGateSerializer, build_priv_tx_circuit};
use tessera_server::aggregator_service::PrivTxAggregator;
...
fn main() -> Result<()> {
	...
	let root: PathBuf = std::env::var("TESSERA_PRIV_TX_ARTIFACTS_PATH")
		.map(PathBuf::from)
	...
	let agg = if PrivTxAggregator::has_full_artifacts(&root)? {
		...
		PrivTxAggregator::from_artifacts(&root, &TesseraGateSerializer)?
	} else {
		...
		agg.store_artifacts(&root, &TesseraGateSerializer)?;
		agg
	};
